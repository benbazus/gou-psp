// src/routes/app/mobile/reconciliation.tsx
...
export default function MobileReconciliationPage() {
  ...
  const { data: settlements = [], isLoading: sLoading } = useQuery({
    queryKey: ['mob-set', tenantId],
    queryFn: () => tenantService.getMobileSettlements(tenantId),
  })

  const isLoading = tLoading || sLoading

  const completed = txns.filter((t) => t.status === 'completed')
  const matched   = completed.filter((_, i) => i % 8 !== 5)
  const unmatched = completed.filter((_, i) => i % 8 === 5)
  ...
  const cards = [
    { label: 'Completed Transactions', value: completed.length,        color: '#3b82f6' },
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-4">
      <motion.div variants={fadeInUp}>
        <PageHeader
          ...
          subtitle="Mobile money transaction reconciliation against settlement records"
          actions={
            <button
        ...
        <motion.div variants={fadeInUp} className="bg-card rounded-card shadow-card overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted">Completed transactions with no matching settlement record</p>
            </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-surface border-b border-border">
                  {['Reference', 'Sender', 'Type', 'Amount', 'Timestamp'].map((h) => (
              ...
              <tbody className="divide-y divide-border">
                {unmatched.slice(0, 10).map((t) => (
                  <tr key={t.id} className="hover:bg-surface/60 transition-colors">
                    ...
                    <td className="px-4 py-3 uppercase text-slate-600">{t.type}</td>
