...
function useSettlementCountdown() {
  ...
  useEffect(() => {
    function calc() {
      ...
      setSecondsLeft(
        Math.max(0, Math.floor((eod.getTime() - now.getTime()) / 1000)),
      );
    }
    calc();
...
function TreasuryStatus() {
  ...
  const pending = mockSettlementBatches.filter(
    (b) => b.status === "pending",
  ).length;
  const processing = mockSettlementBatches.filter(
    (b) => b.status === "processing",
  ).length;
  const completed = mockSettlementBatches.filter(
    (b) => b.status === "completed",
  ).length;
  const failed = mockSettlementBatches.filter(
    (b) => b.status === "failed" || b.status === "rejected",
  ).length;

  const treasury = mockSettlementAccounts.find((a) => a.type === "Treasury");
  const totalGross = mockSettlementBatches.reduce(
    (s, b) => s + b.grossAmount,
    0,
  );
    ...
    <div className="bg-card rounded-card shadow-card p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-muted">
            End-of-day settlement window - 23:00 EAT cutoff
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        {[
          ...
          <div
            key={label}
            className={clsx("rounded-xl border p-3 text-center", color)}
          >
      ...
      <div className="mb-4">
        <div className="flex justify-between text-xs text-muted mb-1.5">
          ...
          <span>
            {doneCount} / {totalCount} batches settled
          </span>
        </div>
        ...
        {treasury && (
          <div className="bg-green-50 rounded-xl p-3 border border-green-200">
            ...
            <div className="text-sm font-bold text-green-700">
              {formatUGX(treasury.balance)}
            </div>
            {treasury.pendingInflow > 0 && (
              <div className="text-[10px] text-green-600 mt-0.5">
                +{formatUGX(treasury.pendingInflow)} inflow
              </div>
            )}
          </div>
        )}
...
function LiveKpiRow({
  ...
  const liveCompleted = liveTransactions.filter(
    (t) => t.status === "completed",
  ).length;
  const liveFailed = liveTransactions.filter(
    (t) => t.status === "failed",
  ).length;
  const liveValue = liveTransactions
    ...
    <>
      ...
      <motion.div variants={fadeInUp}>
        <KPICard
          ...
          value={totalFailed}
          subtitle={
            liveFailed > 0 ? `+${liveFailed} since load` : "Require attention"
          }
      ...
      >
        {isLoading ? (
          ...
          <div className="col-span-4">
            <ErrorState
              kind="network"
              message="Unable to load dashboard statistics. Check your connection and try again."
              compact
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-card rounded-card shadow-card p-4">
          ...
          <p className="text-xs text-muted mb-3">
            Transaction count by channel today
          </p>
            ...
            {channelBreakdown.map((c, i) => (
              <div key={c.channel} className="flex items-center gap-2 text-xs">
                <span
                  ...
                  style={{ background: channelPie[i]?.color }}
                />
        ...
        <div className="bg-card rounded-card shadow-card p-4">
          ...
          <p className="text-xs text-muted mb-3">
            UGX billions collected this month
          </p>
          <BarChart
            data={agencyBar}
            xKey="agency"
            bars={[
              { key: "revenue", color: "#1B3A6B", name: "Revenue (B UGX)" },
            ]}
          />
        </div>
      </div>
