  ...
  return (
    <div className="grid grid-cols-5 gap-3 mb-5">
      {[
        ...
        >
          <span
            className={clsx("w-2.5 h-2.5 rounded-full flex-shrink-0", dot)}
          />
          <div>
    ...
    <motion.tr
      ...
      className={clsx(
        ...
        p.status === "suspended"
          ...
          : p.status === "onboarding"
            ? "bg-yellow-50/20 hover:bg-yellow-50"
            : "hover:bg-primary-50",
      )}
    >
      {/* Name + type */}
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-1.5">
          ...
          <span
            className={clsx("text-xs font-semibold", HEALTH_COLOR[p.apiHealth])}
          >
            {p.apiHealth}
          </span>
          {p.apiHealth !== "down" && (
      ...
      <td className="px-4 py-3">
        ...
        {p.dailyCount > 0 && (
          <div className="text-[10px] text-muted">
            {p.dailyCount.toLocaleString()} txns
          </div>
        )}
      </td>
...
const EMPTY_FORM: OnboardForm = {
  ...
  webhookUrl: "",
  apiVersion: "v1",
  settlementBank: BANKS_LIST[0],
  settlementAccount: "",
  nettingType: NETTING_TYPES[0],
  cutoffTime: "21:00",
...
function OnboardModal({
  ...
  async function submit() {
    ...
    pushSecurityEvent(
      "CONFIG_CHANGED",
      `New participant onboarded: ${form.name}`,
      form.shortName,
    );
  }
    ...
    <Modal
      ...
      title={
        submitted
          ...
          : `Onboard New Participant Step ${step + 1} of 4`
      }
      footer={
        submitted ? (
          <button
      ...
      {submitted ? (
        <div className="text-center py-6">
          ...
          <p className="text-sm text-muted mb-4">
            Successfully onboarded as <strong>{form.type}</strong>
          </p>
          ...
          <p className="text-xs text-muted mt-4">
            API integration testing will begin automatically. Status will update
            to <strong>Active</strong> once certification is complete.
          </p>
        </div>
      ) : (
                ...
                >
                  ...
                  >
                    {i < step ? (
                      ...
                      <Icon
                        size={13}
                        className={i === step ? "text-white" : "text-muted"}
                      />
                    )}
                  </div>
                  <span
              ...
              >
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Institution Name *" error={errors.name}>
                    <input
                      ...
                      placeholder="e.g. Equity Bank Uganda"
                ...
                <Field label="Participant Type *">
                  <select
                    value={form.type}
                    onChange={(e) =>
                      update("type", e.target.value as ParticipantType)
                    }
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Contact Email *" error={errors.email}>
                    <input
                      ...
                      placeholder="tech@institution.ug"
            ...
            {step === 1 && (
              ...
              >
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs text-blue-700 flex items-start gap-2">
                  ...
                  All API connections require TLS 1.3 and HMAC-SHA256 request
                  signing. mTLS client certificates will be issued after
                  onboarding.
                </div>
                <Field
                ...
                >
                  <input
                    ...
                    placeholder="https://api.institution.ug/govpay"
                    className={
                      inputCls(!!errors.apiEndpoint) + " font-mono text-[12px]"
                    }
                  />
                </Field>
                <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Webhook Callback URL">
                  <input
                    ...
                    placeholder="https://institution.ug/webhooks/govpay"
                    className={inputCls(false) + " font-mono text-[12px]"}
                  />
                </Field>
                <div className="bg-surface border border-border rounded-xl p-3 text-xs text-muted">
                  <p className="font-semibold text-slate-700 mb-1">
                    API credentials will be generated after submission:
                  </p>
              ...
              >
                <Field label="Settlement Bank at BOU">
                  <select
                    ...
                    onChange={(e) => update("settlementBank", e.target.value)}
                    className={inputCls(false) + " bg-white"}
                  >
                    {BANKS_LIST.map((b) => (
                ...
                <Field
                  label="BOU Settlement Account Number *"
                ...
                >
                  <input
                    ...
                    placeholder="BOU-XXX-001"
                    className={
                      inputCls(!!errors.settlementAccount) + " font-mono"
                    }
                  />
                </Field>
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Netting Type">
                    <select
                      value={form.nettingType}
                      onChange={(e) => update("nettingType", e.target.value)}
                      className={inputCls(false) + " bg-white"}
                    >
                      {NETTING_TYPES.map((n) => (
            ...
            {step === 3 && (
              ...
              >
                <div className="bg-green-50 border border-green-200 rounded-xl p-3 text-xs text-green-700 flex items-center gap-2">
                  <CheckCircle2 size={12} /> Please review all details carefully
                  before submitting.
                </div>
                ...
                {[
                  ...
                  },
                  {
                    ...
                    rows: [
                      ...
                      {
                        label: "Health URL",
                        value: form.healthCheckUrl || "/health",
                      },
                  ...
                  },
                  {
                    ...
                    rows: [
                      ...
                      { label: "Cutoff Time", value: form.cutoffTime + " EAT" },
                      {
                        ...
                        value: form.minSettlement
                          ? `UGX ${Number(form.minSettlement).toLocaleString()}`
                          : "-",
                      },
                    ],
                  },
                    ...
                    <div className="px-3 py-2 space-y-1.5">
                      {rows.map(({ label, value }) => (
                        ...
                        >
                          ...
                          <span className="font-medium text-slate-800 max-w-[55%] text-right truncate">
                            {value}
                          </span>
                        </div>
                      ))}
...
export default function ParticipantsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "active" | "suspended" | "onboarding"
  >("all");
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          {TYPE_TABS.map((t) => {
            ...
            return (
              ...
              >
                <meta.icon
                  size={12}
                  className={typeFilter === t ? meta.color : ""}
                />
                {TYPE_LABELS[t]}
                <span
                  className={clsx(
        ...
        <div className="flex items-center gap-2 bg-surface border border-border rounded-xl px-3 py-2 ml-auto">
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              className="text-muted hover:text-slate-800 text-[10px]"
            >
              ✕
            </button>
          )}
      ...
      <div className="bg-card rounded-card shadow-card overflow-hidden">
        ...
        {data.some((p) => p.status === "suspended") && (
          <div className="px-4 py-2.5 bg-red-50 border-t border-red-100 flex items-center gap-2 text-xs text-red-700">
            ...
            {data.filter((p) => p.status === "suspended").length} participant(s)
            currently suspended - all transactions blocked until reactivated
          </div>
        )}
        ...
        {data.some((p) => p.status === "onboarding") && (
          <div className="px-4 py-2.5 bg-yellow-50 border-t border-yellow-100 flex items-center gap-2 text-xs text-yellow-700">
            <Activity size={13} />
            {data.filter((p) => p.status === "onboarding").length}{" "}
            participant(s) in onboarding - API integration testing in progress
          </div>
        )}
      </div>
