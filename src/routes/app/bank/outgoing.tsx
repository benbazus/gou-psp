...
export default function BankOutgoingPage() {
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-6">
      <motion.div variants={fadeInUp}>
        <PageHeader
          ...
          subtitle={`${tenantName} — Outbound transaction records`}
        />
      </motion.div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-surface border-b border-border">
                ...
                <th className="px-4 py-2.5 text-left font-medium text-muted uppercase tracking-wider">Payee</th>
              ...
              {isLoading ? (
                ...
                filtered.map((t) => (
                  <tr key={t.id} className="hover:bg-surface/60 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-700">{t.payee}</td>
                    <td className="px-4 py-3 text-right font-semibold text-orange-500">{formatUGX(t.amount)}</td>
